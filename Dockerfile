...
# `libgomp1` (GNU OpenMP runtime) and `libatomic1` are runtime dependencies of onnxruntime-node's
# prebuilt `libonnxruntime.so.1` (THOTH-086's workspace search indexing, via
# @huggingface/transformers). `-slim` images don't ship them, so without them dlopen() fails at
# runtime with `ERR_DLOPEN_FAILED` even though the base is glibc (Debian), not musl.
RUN apt-get update && apt-get install -y --no-install-recommends wget libgomp1 libatomic1 \
    && rm -rf /var/lib/apt/lists/* && \
