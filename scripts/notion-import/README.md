...
Thoth's public `/api/v1/*` HTTP API. It is **not** part of the Thoth application — it requires
zero Thoth code changes and stores all of its own state (Notion↔Thoth id mappings, content
...
- A Thoth **App API key** ([THOTH-026](#)) with `read_write` permission and `workspace` scope
...
| `IMPORT_MODE`              | no       | `auto` (default) / `initial` / `sync`                                                       |
...
*/15 * * * *  cd /opt/notion-import && \
  ...
  STATE_FILE=/opt/notion-import/ws_123.json pnpm notion-import >> import.log 2>&1
```
...
THOTH_WORKSPACE_ID=ws_123 STATE_FILE=./ws_123.json pnpm notion-import
```
...
```bash
pnpm test:unit -- scripts/notion-import   # unit + integration tests for this script
pnpm lint                                 # eslint / tsc / prettier (covers scripts/)
```
