...
// Target-discriminated: returns the reconstructed state at the chosen revision alongside its
// immediately preceding sibling revision's state, so the client can render either a markdown
// char-diff (content) or a per-column before/after table (values). Raw content is only ever
// returned here, for the one revision the caller explicitly opened — never from the list endpoint.
export const getPageRevisionResponseSchema = z.discriminatedUnion('target', [
  ...
  z.object({
    ...
    previousValues: z.record(z.string(), pageValueSchema),
    isFirstRevision: z.boolean(),
