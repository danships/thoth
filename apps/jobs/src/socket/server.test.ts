...
import { z } from 'zod';
import { JobSocketServer } from './server';
...
describe('JobSocketServer', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(nodePath.join(tmpdir(), 'thoth-jobs-socket-test-'));
  });
  ...
  test('binds with a private parent directory (0700) and socket mode (0600)', async () => {
    const socketPath = nodePath.join(dir, 'nested', 'jobs.sock');
  ...
  test('refuses to bind to a path that is a symlink', async () => {
    const socketPath = nodePath.join(dir, 'jobs.sock');
    const targetPath = nodePath.join(dir, 'target-file');
