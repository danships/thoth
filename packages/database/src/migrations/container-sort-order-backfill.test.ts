...
describe('container-sort-order-backfill', () => {
  ...
  beforeAll(async () => {
    ...
    // Explicit, isolated context with sync enabled (mirrors the migration CLI) so the test can
    // invoke `backfillContainerSortOrder` directly and control exactly when it runs relative to
    // seeded rows, without going through the versioned migration runner (which already includes
    // this backfill).
